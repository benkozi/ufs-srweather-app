...
from smoke_dust.core.cycle import SmokeDustCycleTwo, _iter_restart_files_
...
def test_iter_restart_files_(tmp_path: Path) -> None:
    ...
    for root_dir in [outdir, tmp_path]:
        print(root_dir)
        restart_files = list(_iter_restart_files_(root_dir, expected_vars))
